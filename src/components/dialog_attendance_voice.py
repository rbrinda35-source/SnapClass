...
def voice_attendance_dialog(selected_subject_id):
    st.write("Record audio of students saying the attendance phrase, then submit to process.")
    
    audio_data = None
    
    audio_data = st.audio_input("Record Attendance Audio", key="attendance_audio")
    ...
    if st.button("Analyze Audio", type="primary", width="stretch"):
        with st.spinner("Processing audio data..."):
            ...
            if not candidates_dict:
                st.error("No enrolled students have voice data for recognition.")
                return
