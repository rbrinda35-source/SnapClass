...
from src.database.db import check_teacher_exists, create_teacher, teacher_login, get_student_subjects, get_student_attendance_logs, unroll_student_to_subject
import numpy as np
...
def student_dashboard():
    ...
    for i, sub_node in enumerate(subjects):
        ...
        def unenroll_btn():
            if st.button("Unenroll for this course", type="tertiary", width="stretch", icon=":material/delete_forever:"):
        ...
        with cols[i%2]:
            subject_card(name = sub['name'], code = sub['subject_code'], section = sub["section"],
                         stats = [("📆", "Total Classes", stats["total"]), ("✅", "Attended", stats["attended"])],
...
def student_screen():
    ...
    if photo_source:
        ...
        with st.spinner("AL is scanning..."):
    ...
    if show_registration:
        with st.container(border=True):
            ...
            new_name = st.text_input(
                "Enter your name", placeholder="E.g. Swapnil Sharma"
            )
            ...
            try:
                ...
                st.error("Audio data has failed!")
